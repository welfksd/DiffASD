...
class ResBlock(EmbedBlock):
    def __init__(
        ...
        self.tembed_layers = nn.Sequential(
                nn.SiLU(),
                nn.Linear(time_embed_dim, out_channels)
                )
        self.cembed_layers = nn.Sequential(
                nn.SiLU(),
                nn.Linear(time_embed_dim, out_channels)
        )
    ...
    def forward(self, x, temb, cemb):
        if self.updown:
            ...
            h = self.in_layers(x)
        temb_out = self.tembed_layers(temb).type(h.dtype)
        cemb_out = self.cembed_layers(cemb).type(h.dtype)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # condition_embed_dim = base_channels * 4  # 这里设置condition的dim
        ...
        self.condition_embedding = ConditionalEmbedding(num_labels=num_ids, d_model=4, dim=4)
        self.condition_projector = nn.Sequential(
                nn.Linear(condition_embed_dim, time_embed_dim),
                nn.SiLU(),
                nn.Linear(time_embed_dim, time_embed_dim),
                )
        

        ch = channel_mults[0] * base_channels
        self.down = nn.ModuleList(
                [EmbedSequential(nn.Conv2d(self.in_channels, base_channels, 3, padding=1))]
                )
        channels = [ch]
        ds = 1
        for i, mult in enumerate(channel_mults):
            ...
            for _ in range(num_res_blocks):
                layers = [ResBlock(
                        ...
                        out_channels=base_channels * mult,  # 这里进行了修改
                        dropout=dropout,
                        )]
                ...
                self.down.append(EmbedSequential(*layers))
                channels.append(ch)
        ...
        for i, mult in reversed(list(enumerate(channel_mults))):
            for j in range(num_res_blocks + 1):
                ...
                if i and j == num_res_blocks:
                    ...
                    ds //= 2
                self.up.append(EmbedSequential(*layers))
    ...
    def forward(self, x, time, condition, need_condition=True):

        time_embed = self.time_projector(time)
        if need_condition:
            condition_embed = self.condition_projector(self.condition_embedding(condition))
        else:
            condition_embed = self.condition_projector(torch.zeros_like(self.condition_embedding(condition), device=x.device))

        skips = []

        h = x.type(self.dtype)
        for i, module in enumerate(self.down):
            h = module(h, time_embed, condition_embed)
            skips.append(h)
        h = self.middle(h, time_embed, condition_embed)
